...
@section('content')
                ...
                <div class="box-body">
                    <div id="data-table_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                        ...
                        @if($isFilteredByForeignKey)
                            <a href="{{ route($resourceConfig['parentModel']['listingButtonRoute'], request()->query($resourceConfig['parentModel']['foreignKey'])) }}"
                               class="btn btn-primary pull-left parent-model-btn">{{ $resourceConfig['parentModel']['listingButtonLabel'] }}</a>
                        @endif
                    </div>
                </div>
            </div>
    ...
    @if(isset($restrictRoute))
        <div class="modal modal-info fade" id="restrict-modal" tabindex="-1" role="dialog"
             aria-labelledby="restrictModal">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{{ trans('ignicms::admin.restrictTitle') }}</h4>
                    </div>
                    <div class="modal-body">
                        <p>
                            {{ trans('ignicms::admin.restrictConfirm') }}
                        </p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline pull-left" data-target="#restrict-modal"
                                data-dismiss="modal">{{ trans('ignicms::admin.close') }}</button>
                        <form method="POST" action="" class="restrict-form">
                            <input type="hidden" name="_token" value="{!! csrf_token() !!}"/>
                            <input type="hidden" name="_method" value="PUT"/>

                            <button type="submit" type="button" class="restrict-btn btn btn-outline">
                                {{ trans('ignicms::admin.agreeRestriction') }}
                            </button>
                        </form>
                    </div>
                </div>
...
@push('additionalScripts')
    <script type="text/javascript">
        ...
        // Restrict entity
        $('body').on('click', '.js-open-restrict-modal', function (e) {
            e.preventDefault();
            var that = $(this),
                $restrictModal = $('#restrict-modal'),
                restrictURL = that.data('restrict-url');
